...
def load_results(input_dir: Path) -> List[Tuple[float, float, float, float]]:
...
def _series(
    ...
    return lambdas, discrepancy, train_error, test_error
...
def plot_single(rows: List[Tuple[float, float, float, float]], output_path: Path) -> None:
    lambdas, discrepancy, train_error, test_error = _series(rows)
...
def plot_compare(
    rows_a: List[Tuple[float, float, float, float]],
    rows_b: List[Tuple[float, float, float, float]],
    output_path: Path,
    label_a: str,
    label_b: str,
) -> None:
    lambdas_a, discrepancy_a, train_error_a, test_error_a = _series(rows_a)
    lambdas_b, discrepancy_b, train_error_b, test_error_b = _series(rows_b)

    disc_min, disc_max = _global_limits([discrepancy_a, discrepancy_b])
    err_min, err_max = _global_limits(
        [test_error_a, test_error_b, train_error_a, train_error_b]
    )
    ...
    ax1b.set_ylabel("test_error", color="#d62728")
    ...
    ax2b.set_ylabel("test_error", color="#d62728")
    ax2b.tick_params(axis="y", labelcolor="#d62728")
    if err_min is not None and err_max is not None:
        ax2b.set_ylim(err_min, err_max)

    fig.suptitle("Size-shift: discrepancy and test error vs lambda_mix")
    fig.tight_layout()
    fig.savefig(output_path, dpi=200)
